...
## 사전 준비
### 1. 환경설정
- Java 21, SpringBoot 3 환경에서 동작하며, Kafka와 Redis 및 RDB가 필요합니다.
### 2. Events 클래스 초기화
- `Events`는 `@Component` + `ApplicationContextAware`로 등록되어 있어, 스프링 컨텍스트 로딩 시 자동 초기화됩니다.
- 별도의 `@Configuration`이나 초기화 코드 없이 도메인 어디서든 정적 메서드를 호출하면 됩니다.

```java
Events.trigger(new SomeDomainEvent(...));
```
